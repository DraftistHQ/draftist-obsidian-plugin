...
type Config = {
    imagePrefix: string
    isNormalized: (filename: string) => boolean
    cover: {
        prefix: string
        updateFrontmatter: (
            app: Obsidian.App,
            file: Obsidian.TFile,
            fn: (meta: Record<string, unknown>) => void,
        ) => Promise<unknown>
    } | null
}
...
const BLOG_CONFIG: Config = {
    ...
    isNormalized: Post.isNormalizedImageFilename,
    cover: {
        prefix: Post.IMAGE_PREFIX_COVER,
        updateFrontmatter: Post.updateFrontmatter,
    },
}

const DOCS_CONFIG: Config = {
    imagePrefix: Doc.IMAGE_PREFIX_DOC,
    isNormalized: Doc.isNormalizedImageFilename,
    cover: null,
}
    ...
    plugin.addCommand({
        ...
        checkCallback: (checking: boolean) => {
            ...
            switch (result.data.module.kind) {
                case "blog": {
                    if (!checking) {
                        normalizeImages(plugin.app, file, BLOG_CONFIG)
                    }
                    return true
                }
                ...
                case "docs": {
                    if (!checking) {
                        normalizeImages(plugin.app, file, DOCS_CONFIG)
                    }
                    return true
                }
...
class NormalizeImagesModal extends Obsidian.Modal {
    ...
    onOpen() {
        ...
        contentEl.createEl("p", {
            text: "All images used in this document will be collected in the ./images folder next to the document file. If an image is already in this folder, it will be renamed by appending a random hash to guarantee uniqueness. If an image is not in this folder, it will be copied there, renamed, and the link(s) in the document will be updated.",
        })
        ...
        if (imageFile && Image.ALLOWED_FORMATS.includes(imageFile.extension.toLowerCase())) {
            ...
            if (existing) {
                ...
                imagesMap.set(imageFile.path, {
                    ...
                    isInImagesFolder,
                    isNormalized: isInImagesFolder && config.isNormalized(imageFile.name),
                })
            }
        }
    })
...
function triageImages(images: UniqueImage[], imagesFolderPath: string, config: Config): TriagedImage[] {
    const triaged: TriagedImage[] = []

    for (const image of images) {
        if (image.isNormalized) {
            // Already properly named and in correct location - skipping
            triaged.push({
                original: image,
        ...
        const prefix = image.isCover && config.cover ? config.cover.prefix : config.imagePrefix
...
// --- File operations and link updates
...
async function updateCoverLink(
    ...
    await config.updateFrontmatter(app, file, fm => {
...
async function executeNormalization(
    ...
    if (config.cover) {
        await updateCoverLink(app, file, fileCache, replacements, config.cover)
    }

    Notice.info(`Normalized ${toProcess.length} image(s)`)
}

async function normalizeImages(app: Obsidian.App, file: Obsidian.TFile, config: Config): Promise<void> {
    const folder = file.parent
    if (!folder) {
        Notice.warning("Cannot determine page folder")
        return
    }
    ...
    try {
        ...
        const images = collectImages(app, file, fileCache, imagesFolderPath, config)
        ...
        const triaged = triageImages(images, imagesFolderPath, config)
        ...
        new NormalizeImagesModal(app, toProcess.length, async () => {
            try {
                await executeNormalization(app, file, fileCache, imagesFolderPath, triaged, config)
            } catch (error) {
